...
export function likeDiamond(): void{
  describe('Diamond Test', async function () {
    ...
    it('3. add most functions and facets', async () => {
        ...
        assert.sameMembers(facets[findAddressPositionInFacets(addresses[0], facets)][1], getSelectors(diamondLoupeFacet).signatures)
        assert.sameMembers(facets[findAddressPositionInFacets(addresses[1], facets)][1], getSelectors(ercDiamondFacet).signatures)
